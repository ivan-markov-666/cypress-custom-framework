...
export class fillWithTextInputTextElement {
   ...
   * @param sendKeysText              Provide the text that will be filled into the input text element.
  ...
  fillWithTextInputTextElement(
    ...
    var functionName: any = "fillWithTextInputTextElement"; // Declare the variable that assigns the name of the function.
    ...
    try {
      ...
      cy.element(elementSelector, element, 20000).sendKeys(sendKeysText);

      // Logging messages.
      if (conf.mostUsedAutomationTestStepsMessages == "enable") {
        cy.log(
          conf.customCommandsGlobalMessage +
            "This step ware executed correctly. The following text was filled into the input text element: '" +
            sendKeysText +
            "'."
        );
      }
