...
export class verifyAttributeValueIsCorrect {
   ...
   * @param element                   Provide the element that will be inspected.
   * @param attribute                 Provide the attribute that will be inspected.
  ...
  verifyAttributeValueIsCorrect(
    ...
    var functionName: any = "verifyAttributeValueIsCorrect"; // Declare the variable that assigns the name of the function.
    ...
    try {
      ...
      cy.element(elementSelector, element, 20000).verifyAttributeContainsValue(
        attribute,
      ...
      if (conf.mostUsedAutomationTestStepsMessages == "enable") {
        cy.log(
          conf.customCommandsGlobalMessage +
            "This step ware executed correctly. The '" +
            attribute +
