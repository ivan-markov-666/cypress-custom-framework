...
export class dropDownListByClick {
  ...
  /**
   * @description                               This function selects a value from the drop-down list by clickings.
   * @param elementSelectorDropDownList         Provide the drop-down list selector that we are using. It can be 'css' for the CSS selector, 'xpath' for the XPATH selector and 'contains' for containing text.
   * @param dropDownListElement                 Provide the drop-down list element.
   * @param elementSelectorDropDownValue        Provide the drop-down value selector that we are using. It can be 'css' for the CSS selector, 'xpath' for the XPATH selector and 'contains' for containing text.
   * @param dropDownValueElement                Provide the drop-down value element.
   * @param verifyElementSelectorDropDownValue  Provide the verification drop-down value selector that we are using. It can be 'css' for the CSS selector, 'xpath' for the XPATH selector and 'contains' for containing text.
   * @param verifydropDownValueElement          Provide the verification drop-down value element.
   * @param verifyDropDownListValue             Provide the drop-down list value (text) that we selected.
   ...
   */
  dropDownListByClick(
    elementSelectorDropDownList: string,
    dropDownListElement: string,
    elementSelectorDropDownValue: string,
    dropDownValueElement: string,
    verifyElementSelectorDropDownValue: string,
    verifydropDownValueElement: string,
    verifyDropDownListValue: string,
    testCaseStepsList?: string[],
    firstTestCaseStep?: number,
    lastTestCaseStep?: number
  ) {
    //05. Define variables used from the functions.
    var functionName: any = "dropDownListByClick"; // Declare the variable that assigns the name of the function.
    ...
    try {
      ...
      // Find the drop-down list and click it.
      cy.element(elementSelectorDropDownList, dropDownListElement).click();
      // Click on the value of the drop-down list...
      cy.element(elementSelectorDropDownValue, dropDownValueElement)
        .click({force: true})
        ...
        .then(() =>
          this.verifyAfter_selectCityDropDownList(
            verifyElementSelectorDropDownValue,
            verifydropDownValueElement,
            verifyDropDownListValue
          )
        );
    } catch (error) {
      // Unit Test.
  ...
  verifyAfter_selectCityDropDownList(
    verifyElementSelectorDropDownValue: string,
    verifydropDownValueElement: string,
    verifyDropDownListValue: string
  ) {
    cy.element(
      verifyElementSelectorDropDownValue,
      verifydropDownValueElement
    ).verifyContainsText(verifyDropDownListValue);
    // Logging messages.
    if (conf.mostUsedAutomationTestStepsMessages == "enable") {
      cy.log(
        conf.customCommandsGlobalMessage +
          "This step ware executed correctly. The user was able to select " +
          verifyDropDownListValue +
