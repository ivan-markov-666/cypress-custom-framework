...
export class pressKeyboardKey {
   ...
   * @param sendKeyboardKeysText      Provide the keys that we want to press. For more details please refer to official documentation: https://docs.cypress.io/api/commands/type#Arguments .
  ...
  pressKeyboardKey(
    ...
    element: string,
    sendKeyboardKeysText: string,
    testCaseStepsList?: string[],
    firstTestCaseStep?: number,
    lastTestCaseStep?: number
  ) {
    //05. Define variables used from the functions.
    var functionName: any = "pressKeyboardKey"; // Declare the variable that assigns the name of the function.
    ...
    try {
      ...
      cy.element(elementSelector, element, 20000).sendKeyboardKey(
        sendKeyboardKeysText
      );
      ...
      if (conf.mostUsedAutomationTestStepsMessages == "enable") {
        cy.log(
          conf.customCommandsGlobalMessage +
            "This step ware executed correctly. The following key/s was pressed: '" +
            sendKeyboardKeysText +
            "'."
        );
      }
