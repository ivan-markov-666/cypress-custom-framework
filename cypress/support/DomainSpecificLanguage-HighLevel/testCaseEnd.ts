...
export class testCaseEnd {
   ...
   */
  testCaseEnd(testCaseID: string) {
    //05. Define variables used from the functions.
    var functionName: any = "testCaseEnd"; // Declare the variable that assigns the name of the function.
    // Create a try-catch statement. If the function fails - we will receive an error message.
    try {
      // Call the function responsible for printing the end of executing test case to the Cypress Test Runner body.
      custFunc.finishTestCase(testCaseID);
