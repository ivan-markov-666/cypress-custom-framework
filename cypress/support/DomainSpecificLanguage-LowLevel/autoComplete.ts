...
var functionName: any = "autoComplete";
...
Cypress.Commands.add(
  ...
  (
    ...
    try {
      ...
      let attributeValue = "";
      cy.wrap(subject).type(sendText);
      cy.element(locatorType, selector).each(($element) => {
        if ($element.text() === selectValue) {
      ...
      if (conf.commandMessages == "enable") {
        cy.log(
          conf.cypressCustomCommandsGlobalMessage +
            "The element was auto complected."
        );
      }
    } catch (error) {
      // Unit Test.
