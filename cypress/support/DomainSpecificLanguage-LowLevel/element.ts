...
var functionName: any = "element";
// Create a Cypress parent custom command.
Cypress.Commands.add(
  functionName,
  (locatorType: string, selector: string, timeoutValue?: number) => {
    ...
    try {
      ...
      if (locatorType == "css") {
        ...
        if (conf.commandMessages == "enable") {
          cy.log(
            conf.cypressCustomCommandsGlobalMessage +
              "The element was selected by CSS selector."
          );
        }
        cy.get(selector, { timeout: timeoutValue })
          // Make assertions.
          .should("exist")
          .and("not.be.disabled")
          .and("be.visible")
        ...
        if (conf.commandMessages == "enable") {
          cy.log(
            conf.cypressCustomCommandsGlobalMessage +
              "The element was selected by XPATH selector."
          );
        }
        cy.xpath(selector, { timeout: timeoutValue })
          // Make assertions.
          .should("exist")
          .and("not.be.disabled")
          .and("be.visible")
        ...
        cy.contains(selector, { timeout: timeoutValue })
          // Make assertions.
          .should("exist")
          .and("not.be.disabled")
          .and("be.visible")
