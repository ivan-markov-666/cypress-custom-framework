...
//01. Import classes.
import { FillTheForm_02_po } from "../../pom/0.0.Examples/3.RealExample/FillTheForm_02_po"; // Import PO class.

//02. Declare constructors.
const po = new FillTheForm_02_po(); // PO class constructor.
...
describe("Fill the form", async () => {
  ...
  await it("Fill the form with valid data.", () => {
    ...
    cy.get(po.firstName_InputTextElement).type("Tester");
    // 3. Fill with valid data into the "Last Name" input text element.
    cy.get(po.lastName_InputTextElement).type("Testerov");
    // 4. Fill with accurate data into the "Email" input text element.
    cy.get(po.email_InputTextElement).type("testingemail@testemail.com");
    ...
    cy.get(po.mobile_InputTextElement).type("1232131231");
    // 7. Fill with accurate data into the "Date of Birth" input text element.
    cy.get(po.dateOfBirth_InputTextElement).click();
    cy.xpath(po.dateOfBirth_SelectValue).click();
    ...
    cy.get(po.currentAddress_InputTextElement).type("Test Address");
