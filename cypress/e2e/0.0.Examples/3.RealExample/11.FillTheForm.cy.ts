 ...
 *                    The extreme practice we can use for the spec (test) class.
 *                    This class is the same as the previous one. The difference is in the PO class.
 ...
 *                      - We use domain-specific language from a low level (custom Cypress commands) not in the spec class, not in the PO class - in the methods, we call in the PO class.
 *                      - We use domain-specific language from a high level (custom methods we call into the PO class) instead of domain-specific language from a low level (custom Cypress commands).
 */

//01. Import classes.
import { baseClass } from "../../0.BaseClass/baseClass"; // Import base test class.
import { FillTheForm_11_po } from "../../pom/0.0.Examples/3.RealExample/FillTheForm_11_po"; // Import PO class.
...
const po = new FillTheForm_11_po(); // PO class constructor.
...
//06. Get test data as a list.
const thisTestCase = po.testCase(baseUrl);
...
describe("Fill the form", async () => {
  ...
  await it("Fill the form with valid data.", () => {
    ...
    // Print the start of the Test Case to the Cypress test runner body.
    po.StartTheTestCase(thisTestCase[0]);

    // 0. Set the screen size to 1920-1080.
    po.precondition(thisTestCase, 1);
    // 1. Navigate to: https://demoqa.com/automation-practice-form .
    po.navigateTo(baseUrl, thisTestCase, 2);
    // 2. Fill with correct data into the "First Name" input text element.
    po.fillFirstNameInputTextElement(firstName, thisTestCase, 3);
    // 3. Fill with valid data into the "Last Name" input text element.
    po.fillLastNameInputTextElement(lastName, thisTestCase, 4);
    // 4. Fill with accurate data into the "Email" input text element.
    po.fillEmailInputTextElement(email, thisTestCase, 5);
    // 5. Select the correct random option from the "Gender" section.
    po.clickGenderRadioOption(thisTestCase, 6);
    // 6. Fill with valid data into the "Mobile Number" input text element.
    po.fillMobileInputTextElement(mobile, thisTestCase, 7);
    // 7. Fill with accurate data into the "Date of Birth" input text element.
    po.fillDateOfBirthInputTextElement(dateOfBirth, thisTestCase, 8);
    // 8. The select random correct date for "Subjects".
    po.fillSubjectInputTextElement(subject, thisTestCase, 9);
    // 9. Check random correct value/s from the "Hobbies" section.
    po.checkHobbiesCheckBox(thisTestCase, 10);
    // 10. Upload a random correct picture file.
    po.uploadPicture(uploadFilePath, thisTestCase, 11);
    // 11. Fill with correct data into the "Current Address" input text element.
    po.fillCurrentAddressInputTextElement(currentAddress, thisTestCase, 12);
    // 12. Select the random correct date for the "State" drop-down list.
    po.selectStateDropDownList(verifySelectedStateDropDownList, thisTestCase, 13);
    // 13. Select an arbitrary valid date for the "City" drop-down list.
    po.selectCityDropDownList(verifySelectedCityDropDownList, thisTestCase, 14);
    // 14. Press the "Submit" button.
    po.pressSubmitButton(thisTestCase, 15);
    // 15. Verify that the test case ware executed everything correctly.
    po.assertionToVerifyFillingWasCorrectlySaved(firstName, lastName, email, gender, mobile, verifyDateOfBirth, subject, hobbies, uploadFile, currentAddress, verifySelectedStateDropDownList, verifySelectedCityDropDownList, thisTestCase, 16, 16);

    // Print the end of the Test Case to the Cypress test runner body.
    po.EndTheTestCase(thisTestCase[0]);
  });
});
