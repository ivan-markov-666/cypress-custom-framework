 ...
 *                          -- The functions contain only Cypress commands. We are not using custom Cypress commands (domain-specific language from a low level).
 */
...
export class FillTheForm_07_po {
  ...
  navigateTo(baseUrl: string) {
    cy.visit(baseUrl+endpoint)
      .url()
      .should("be.equal", baseUrl+endpoint);
  }
  ...
  fillFirstNameInputTextElement(firstName: string) {
    cy.get(this.firstName_InputTextElement)
      .type(firstName)
  ...
  fillLastNameInputTextElement(lastName: string) {
    cy.get(this.lastName_InputTextElement)
      .type(lastName)
  ...
  fillEmailInputTextElement(email: string) {
    cy.get(this.email_InputTextElement).type(email).should("have.value", email);
  }
  ...
  clickGenderRadioOption() {
    cy.xpath(this.genderMale_RadioButton).click();
  }
  ...
  fillMobileInputTextElement(mobile: string) {
    cy.get(this.mobile_InputTextElement)
      .type(mobile)
  ...
  fillDateOfBirthInputTextElement(dateOfBirth: string) {
    cy.get(this.dateOfBirth_InputTextElement).type("{selectall}");
    cy.get(this.dateOfBirth_InputTextElement).type(dateOfBirth);
    cy.get(this.dateOfBirth_InputTextElement).type("{enter}");
  }
  ...
  fillSubjectInputTextElement(subject: string) {
    cy.get(this.subject_InputTextElement).type(subject + "{enter}");
  }
  ...
  checkHobbiesCheckBox() {
    cy.xpath(this.hobbiesSport_CheckBox).click();
  }
  ...
  uploadPicture(uploadFilePath: string) {
    cy.get(this.uploadPicture_Button).attachFile(uploadFilePath);
  }
  ...
  fillCurrentAddressInputTextElement(currentAddress: string) {
    cy.get(this.currentAddress_InputTextElement)
      .type(currentAddress)
  ...
  selectStateDropDownList(verifySelectedStateDropDownList: string) {
    cy.get(this.state_DropDownList).click();
    cy.xpath(this.state_DropDownListSelectedValue).click({force: true});
    cy.xpath(this.verifyStateValue_DropDownList).contains(
  ...
  selectCityDropDownList(verifySelectedCityDropDownList: string) {
    cy.get(this.city_DropDownList).click();
    cy.xpath(this.city_DropDownListSelectedValue).click();
    cy.xpath(this.verifyCityValue_DropDownList).contains(
  ...
  assertionToVerifyFillingWasCorrectlySaved(
    ...
    cy.xpath(this.assertStudentName).contains(firstName + " " + lastName);
    cy.xpath(this.assertStudentEmail).contains(email);
    cy.xpath(this.assertGender).contains(gender);
    cy.xpath(this.assertMobile).contains(mobile);
    cy.xpath(this.assertDateOfBirth).contains(verifyDateOfBirth);
    cy.xpath(this.assertSubjects).contains(subject);
    cy.xpath(this.assertHobbies).contains(hobbies);
    cy.xpath(this.assertPicture).contains(uploadFile);
    cy.xpath(this.assertAddress).contains(currentAddress);
    cy.xpath(this.assertStateNadCity).contains(
