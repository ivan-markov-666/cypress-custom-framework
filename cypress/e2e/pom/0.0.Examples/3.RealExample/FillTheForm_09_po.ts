 ...
 *                      The best practice we can use for the page object class.
 ...
 *                          -- We use custom Cypress commands (domain-specific language from a low level), and the assertion (verification that we executed the steps correctly) is made in a different function.
 */
...
export class FillTheForm_09_po {
  ...
  navigateTo(baseUrl: string) {
    ...
    cy.navigateTo(baseUrl + endpoint)
      // ... and verify that we made the navigation correctly by calling another function.
      .then(() => this.verifyAfter_navigateTo(baseUrl + endpoint));
  }
  // Declare the function where we will make the verification to guarantee the step was executed correctly.
  verifyAfter_navigateTo(verifyUrl: string) {
    // Find the element, and then...
    cy.element("css", this.firstName_InputTextElement).then(() => {
      // ...verify that the URL is like the expected one.
  ...
  fillDateOfBirthInputTextElement(dateOfBirth: string) {
    cy.element("css", this.dateOfBirth_InputTextElement).type(
      "{selectall}"
    );
  ...
  fillSubjectInputTextElement(subject: string) {
    cy.element("css", this.subject_InputTextElement).type(
      subject + "{enter}"
    );
  }
  ...
  selectStateDropDownList(verifySelectedStateDropDownList: string) {
    ...
    cy.element("xpath", this.state_DropDownListSelectedValue)
      .click({force: true})
      // ... and verify that we made the operation correctly by calling another function.
      .then(() =>
  ...
  assertionToVerifyFillingWasCorrectlySaved(
    ...
    cy.element("xpath", this.assertSubjects).verifyContainsText(
      subject
    );
